...
class _RalphLoopBase:
    ...
    async def handle(self, ctx: CommandContext) -> CommandResult | None:
        ...
        if not args_text:
            return CommandResult(
                text=(
                    "Usage:\n"
                    "- `/ralph_loop <task>`\n"
                    "- `/ralph_loop /<project> <task>`\n"
                    "- `/ralph_loop /<project> @<branch> <task>`"
                )
            )
        ...
        if explicit_branch and explicit_branch in banned:
            return CommandResult(
                text=(
                    f"`/ralph_loop` refuses to run on `{explicit_branch}` (base branch).\n\n"
        ...
        if explicit_branch and current_branch and explicit_branch == current_branch:
            return CommandResult(
                text=(
                    f"`/ralph_loop` refuses to run on the project's current branch `{current_branch}`.\n\n"
                    "Use `/ralph_loop @oc/<name> ...` or `/dot ...` if you truly want the main checkout."
                )
            )
        ...
        # Wrap the prompt with /ralph-loop to trigger the oh-my-opencode command
        ralph_prompt = f"/ralph-loop {prompt}"

        await ctx.executor.run_one(RunRequest(prompt=ralph_prompt, context=run_ctx))
        ...
        lines: list[str] = []
        lines.append(
            f"ralph-loop: completion workflow started for `{project}` @ `{branch}`"
        )
        if result.returncode != 0:
            lines.append(
                f"ralph-loop: workflow start failed (exit {result.returncode})"
            )
            if stdout:
                lines.append("")
                lines.append("stdout:")
        ...
        if log_path:
            ...
            lines.append("ralph-loop: workflow started, but couldn't parse log path.")
            lines.append("")
            lines.append(stdout[-3000:])
